...
//                                 $relationship_table_pk => CRUDBooster::newId($ro['relationship_table']),
        ...
        foreach ($rows as $value) {
            ...
            try {
                ...
                if ($has_created_at) {
				  ...
				  if (CRUDBooster::isColumnExists($this->table, 'Tanggal')) {
     					//patch tanggal by periode vaishal
	     				$a['Tanggal'] = Session::get('periode_login');
					}
    ...
    public function postDoUploadImportData()
    {
        ...
        if (Request::hasFile('userfile')) {
			...
			//vaishal clear all data before import
			DB::table($this->table)->whereRaw('MONTH(tanggal) = MONTH("'.Session::get('periode_login').'") AND YEAR(tanggal) = YEAR("'.Session::get('periode_login').'")')->delete();
